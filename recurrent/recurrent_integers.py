...
curriculum_stage_0 = max_n+max_n*max_n+max_n*max_n*max_n 
...
def valid(l):
    ...
    return all(map(lambda x: x is not None,l))
...
valid_points = map(valid,y_data)
y_data = numpy.array(map(list,y_data[valid_points]))
x_data = numpy.array(map(lambda l: map(lambda x: numpy.argmax(x),l),numpy.array(raw_x_data)[valid_points]),dtype=numpy.int32)
...
#	continue
    ...
    embeddings = tf.Variable(tf.random_uniform([max_n,embedding_size],-0.1/embedding_size,0.1/embedding_size))
