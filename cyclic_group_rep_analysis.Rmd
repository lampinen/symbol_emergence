...
reps = data.frame()
for (run in 0:nruns) {
  if (!file.exists(sprintf('results/cyclic_sigmoid/order_%i_nhidden-shared_%i-separate_%i_rseed_%i_final_a_reps.csv',group_order,nhidden_shared,nhidden_separate,run))) 
  {
    next
  }
  a_reps = read.csv(sprintf('results/cyclic_sigmoid/order_%i_nhidden-shared_%i-separate_%i_rseed_%i_final_a_reps.csv',group_order,nhidden_shared,nhidden_separate,run),header=F)
  b_reps = read.csv(sprintf('results/cyclic_sigmoid/order_%i_nhidden-shared_%i-separate_%i_rseed_%i_final_b_reps.csv',group_order,nhidden_shared,nhidden_separate,run),header=F)
...
for (cl in 1:nclusters) {
  ...
  ggsave(sprintf('results/cyclic_sigmoid_plots/order_%i_nhidden-shared_%i-separate_%i_numrunsused_%i_nclusters_%i_cluster_%i.png',group_order,nhidden_shared,nhidden_separate,num_runs_used,nclusters,cl))
}
```
