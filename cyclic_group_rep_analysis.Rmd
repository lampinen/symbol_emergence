...
num_runs_used = 0
reps = data.frame()
for (run in start_run:(start_run+nruns)) {
  if (!file.exists(sprintf('results/cyclic_curriculum/curr_%s_order_%i_nhidden-shared_%i-separate_%i_rseed_%i_final_a_reps.csv',curriculum,group_order,nhidden_shared,nhidden_separate,run))) 
  {
    next
  }
  a_reps = read.csv(sprintf('results/cyclic_curriculum/curr_%s_order_%i_nhidden-shared_%i-separate_%i_rseed_%i_final_a_reps.csv',curriculum,group_order,nhidden_shared,nhidden_separate,run),header=F)
  b_reps = read.csv(sprintf('results/cyclic_curriculum/curr_%s_order_%i_nhidden-shared_%i-separate_%i_rseed_%i_final_b_reps.csv',curriculum,group_order,nhidden_shared,nhidden_separate,run),header=F)
...
rep_simils = reps %>% 
  ...
  spread(var,value) %>%
  group_by(run) %>%
  do(data.frame(similarities=unlist(as.list(simil(.[,(1:group_order)+2]))),indices=1:((group_order*(group_order-1))/2))) %>% #uhhh #.$V7a,.$V7b, indices=1:21
  ungroup() %>%
...
for (cl in 1:nclusters) {
  ...
  ggsave(sprintf('results/cyclic_curriculum_plots/order_%i_nhidden-shared_%i-separate_%i_numrunsused_%i_nclusters_%i_cluster_%i.png',group_order,nhidden_shared,nhidden_separate,num_runs_used,nclusters,cl))
  next
  print(ggplot(PCed_reps,aes(x=PC3,y=PC2,color=element)) +
    geom_point(size=5) +
    theme_bw() +
    ggtitle(sprintf('cluster %i PCs 23',cl)))
  ggsave(sprintf('results/cyclic_curriculum_plots/order_%i_nhidden-shared_%i-separate_%i_numrunsused_%i_nclusters_%i_cluster_%i_PCs23.png',group_order,nhidden_shared,nhidden_separate,num_runs_used,nclusters,cl))
  
  print(ggplot(PCed_reps,aes(x=PC3,y=PC4,color=element)) +
    geom_point(size=5) +
    theme_bw() +
    ggtitle(sprintf('cluster %i PCs 34',cl)))
  ggsave(sprintf('results/cyclic_curriculum_plots/order_%i_nhidden-shared_%i-separate_%i_numrunsused_%i_nclusters_%i_cluster_%i_PCs34.png',group_order,nhidden_shared,nhidden_separate,num_runs_used,nclusters,cl))
}
```
